...
/**
 * POST /api/ml/predict
 * Obtém recomendação de serviço para um cliente
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${ML_API_URL}/ml/predict?tenant=${tenantSlug}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || 'Erro ao obter predição' },
    ...
    console.error('Erro na rota /api/ml/predict:', error);
    return NextResponse.json(
