...
/**
 * GET /api/ml/model
 * Obtém informações sobre o modelo ML atual
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${ML_API_URL}/ml/model?tenant=${tenantSlug}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || 'Erro ao obter informações do modelo' },
    ...
    console.error('Erro na rota /api/ml/model:', error);
    return NextResponse.json(
